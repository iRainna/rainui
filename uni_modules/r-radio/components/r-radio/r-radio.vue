<template>
	...
	>
		...
		<view
			...
			"
		>
			<slot v-if="$slots.icon" name="icon" :checked="checked" :disabled="getProps('disabled')"></slot>
			<r-icon
...
const getProps = (name) => {
	...
	for (let key of allKeys) {
		allProps[key] = allProps[key] == defaultPropsValue[key] ? parentProps.value[key] || props[key] : props[key];
	}
	return allProps[name];
};

const componentsName = 'r-radio';
...
const getComponentThemeStyle = computed(() => {
	...
	if (getProps('themeName') && getProps('themeName') != 'default') {
		//单独设置了组件的 就用单独设置的
