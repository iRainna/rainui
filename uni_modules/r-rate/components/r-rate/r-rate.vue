<template>
	...
	>
		...
		<view
			...
			:style="{
				paddingRight: gutter && index + 1 !== count ? (isNumeric(gutter) ? gutter + 'rpx' : gutter) : 0,
				width: size,
				height: size
			}"
...
const list = computed(() => new Array(props.count).fill('').map((_, i) => getRateStatus(props.value, i + 1, props.allowHalf, props.readonly)));
const updateRanges = () => {
	ranges.value = [];
...
const getScoreByPosition = (x, y) => {
	for (let i = ranges.value.length - 1; i > 0; i--) {
		if (y >= groupRefRect.value.top && y <= groupRefRect.value.bottom) {
			if (x > ranges.value[i].left && y >= ranges.value[i].top && y <= ranges.value[i].top + ranges.value[i].height) {
...
const onClickItem = (event, index) => {
	...
	const score = index + 1;
	updateRanges();
	let value = allowHalf ? getScoreByPosition(event.changedTouches[0].clientX, event.changedTouches[0].clientY) : score;
