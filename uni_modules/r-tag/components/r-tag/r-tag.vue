<template>
	<r-animation :show="show" :entryClass="entryClass" :exitClass="exitClass" :duration="duration" :delay="delay" :customStyle="customTransitionStyle">
		<!-- renderTag -->
		<span
			:class="{
				'r-tag': true,
		...
		>
			<slot />
			<r-icon v-if="closeable" :customClass="`r-tag__close r-haptics-feedback`" name="cross" size="24rpx" :color="plain ? textColor || color : textColor" @click="onClose" />
		</span>
	</r-animation>
</template>
<script setup>
